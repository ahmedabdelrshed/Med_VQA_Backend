  ...
  const updateQuestion = async (req, res, next) => {
    try {
        ...
        if ( question.length < 5) {
            return next(appError.createError("Question must be at least 5 characters long", 400, "VALIDATION_ERROR"));
        }

        if (!questionId) {
            return next(appError.createError("Question ID is required", 400, "VALIDATION_ERROR"));
        }
        ...
        next(appError.createError("An error occurred while updating the question", 500, "SERVER_ERROR", error));
    }
};
...
const deleteQuestion = async (req, res, next) => {
  try {
        ...
        next(appError.createError("An error occurred while deleting the question", 500, "SERVER_ERROR", error));
    }
}
