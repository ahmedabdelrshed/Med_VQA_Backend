...
const validateSymptoms = [
  ...
  body('symptoms.*')
    ...
    .custom(symptom => validSymptoms.includes(symptom))
    .withMessage('One or more symptoms are invalid'),

  (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ status: 'error', errors: errors.array().map(error => error.msg) });
    }
    next();
  }
];
